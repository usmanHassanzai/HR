-- People board: Score = all-time; Month score = current month only (no overall fallback).

CREATE OR REPLACE FUNCTION public.user_overall_kpi_score(p_user_id UUID)
RETURNS NUMERIC
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.user_overall_kpi_score(UUID) TO authenticated;
