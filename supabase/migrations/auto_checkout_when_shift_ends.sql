-- When a person is still checked in after their assigned shift ends, clock them out
-- at the shift end time (Asia/Karachi). Works if they stay on site, close the app, or never ping GPS.
...
REVOKE ALL ON FUNCTION public.close_my_ended_shift_attendance() FROM PUBLIC;
...
BEGIN
    ...
    v_shift_closed := public.close_open_attendance_if_shift_ended(v_user_id, p_latitude, p_longitude);
    ...
    IF v_shift_closed > 0 THEN
        SELECT * INTO v_rec
        FROM public.attendance_records ar
        WHERE ar.user_id = v_user_id
        ORDER BY COALESCE(ar.clock_out_at, ar.clock_in_at) DESC NULLS LAST
        LIMIT 1;
        v_has_rec := FOUND;
        IF v_has_rec THEN
            v_attendance_date := v_rec.attendance_date;
    ...
    ELSIF v_has_rec AND v_rec.clock_in_at IS NOT NULL AND v_rec.clock_out_at IS NULL
