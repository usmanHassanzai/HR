-- Store manager/admin assignment write-up separately so employees can see it.
...
BEGIN
    ...
    SELECT u.email, u.full_name INTO v_email, v_name
    ...
    IF v_other_pending + v_new_weight > 100.05 THEN
        RAISE EXCEPTION 'KPI cannot be assigned. % currently has % assigned KPI weight. This KPI is worth %. New total would be %. Maximum allowed for one employee is 100%%. Remaining capacity: %.',
            v_name,
            round(v_other_pending, 2),
            round(v_new_weight, 2),
            round(v_other_pending + v_new_weight, 2),
            round(GREATEST(0, 100 - v_other_pending), 2);
    END IF;
    ...
    LOOP
        INSERT INTO public.kpis (
            user_id, name, description, assignment_notes, department, department_id, category,
    ...
    PERFORM public.create_system_notification(
        ...
        CASE
            WHEN v_notes IS NOT NULL THEN
                v_count || ' KPI task(s) assigned from ' || v_dept_name || E'.\n\nAssignment details:\n' || v_notes
            ELSE
                v_count || ' KPI task(s) assigned from ' || v_dept_name || '.'
        END,
        'info'
    );
