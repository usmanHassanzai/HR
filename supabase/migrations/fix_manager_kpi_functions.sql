-- Fix missing manager KPI helper functions on live Supabase.
...
BEGIN
    ...
    LOOP
        ...
        IF v_id IS NOT NULL AND EXISTS (
            SELECT 1 FROM public.department_kpi_indicators WHERE id = v_id AND department_id = p_department_id
        ) THEN
            UPDATE public.department_kpi_indicators SET
                weight_pct = v_pct, name = v_name,
                description = COALESCE(v_desc, description),
                sort_order = v_sort, active = true,
        ...
        ELSE
            ...
            ) VALUES (p_department_id, v_name, v_desc, v_pct, v_sort, true)
            ON CONFLICT (department_id, name) DO UPDATE SET
                description = EXCLUDED.description, weight_pct = EXCLUDED.weight_pct,
    ...
    UPDATE public.department_kpi_indicators SET active = false, updated_at = timezone('utc'::text, now())
    WHERE department_id = p_department_id AND active = true AND NOT (id = ANY(v_kept));
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;

CREATE OR REPLACE FUNCTION public.seed_default_department_kpis(p_department_id UUID)
RETURNS INTEGER AS $$
DECLARE v_count INTEGER; v_dept_name TEXT;
BEGIN
    IF NOT public.can_manage_department_kpis(p_department_id) THEN
        RAISE EXCEPTION 'Not authorized to seed KPIs for this department';
    END IF;
    SELECT COUNT(*) INTO v_count FROM public.department_kpi_indicators
    ...
    VALUES
        (p_department_id, 'Performance Target / Volume', 'Actual results versus monthly quota.', 30, 1, true),
        (p_department_id, 'Quality & Accuracy', 'Percentage of work passing quality checks.', 30, 2, true),
        (p_department_id, 'Timeliness / Delivery', 'Tasks completed on time.', 20, 3, true),
        (p_department_id, 'Efficiency & Productivity', 'Average time to complete processes.', 20, 4, true)
