-- Auto check-out only when GPS proves the person left the office radius.
-- Closing the dashboard/app does not check anyone out.
...
BEGIN
    ...
    IF p_distance <= p_effective_radius THEN
        RETURN FALSE;
    END IF;
    v_far := GREATEST(p_effective_radius * 1.75, p_effective_radius + 80);
    IF p_distance >= v_far THEN
        RETURN TRUE;
    END IF;
