-- HR has the same company product access as admin (not platform-owner features).
-- Widen is_admin() to include the hr role so existing RPCs/RLS that check is_admin() apply to HR.
...
BEGIN
    RETURN EXISTS (
        ...
        WHERE u.id = user_id
          AND u.role IN ('admin'::public.user_role, 'hr'::public.user_role)
    );
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public;

-- Keep is_hr for explicit HR-only messaging / shift helpers
...
-- can_manage_org_shifts already = is_admin OR is_hr; after widen, is_admin alone is enough
CREATE OR REPLACE FUNCTION public.can_manage_org_shifts(p_uid UUID DEFAULT auth.uid())
RETURNS BOOLEAN AS $$
    SELECT public.is_admin(p_uid);
$$ LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public;

-- People board: company-wide for any is_admin (admin or HR)
...
BEGIN
    ...
    IF NOT (
        ...
        RAISE EXCEPTION 'Only managers, HR, and admins can view team KPI weightage';
    END IF;
    ...
    WITH staff AS (
        ...
        FROM public.users u
        WHERE u.role IN ('employee'::public.user_role, 'manager'::public.user_role, 'admin'::public.user_role, 'hr'::public.user_role)
