-- Hybrid work: office GPS on site days, remote marking (self or supervisor) on WFH days.
...
BEGIN
    ...
    IF p_work_mode IS NULL OR p_work_mode NOT IN ('office', 'remote', 'hybrid') THEN
        RAISE EXCEPTION 'Work mode must be office, remote, or hybrid';
    END IF;
...
BEGIN
    ...
    SET status = EXCLUDED.status,
        ...
        attendance_source = CASE
            WHEN public.attendance_records.attendance_source = 'geo' THEN public.attendance_records.attendance_source
            ELSE 'manual'
        END
...
-- Hybrid staff: mark a work-from-home day (office days still use GPS).
...
BEGIN
    ...
    VALUES (
        ...
        v_uid,
        v_now,
        CASE WHEN p_status = 'present' THEN v_now END,
        CASE WHEN p_status = 'present' THEN v_now END,
        'manual'
    )
    ON CONFLICT (user_id, attendance_date) DO UPDATE
    SET status = EXCLUDED.status,
        ...
        clock_in_at = CASE
            WHEN EXCLUDED.status = 'absent' THEN NULL
            ELSE COALESCE(public.attendance_records.clock_in_at, EXCLUDED.clock_in_at)
        END,
        clock_out_at = CASE
            WHEN EXCLUDED.status = 'absent' THEN NULL
            ELSE COALESCE(public.attendance_records.clock_out_at, EXCLUDED.clock_out_at)
        END,
