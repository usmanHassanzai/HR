-- Viewed means the assigned employee/manager opened the task in Scorr.
-- Admin/manager edits and email delivery must not mark it viewed.

ALTER TABLE public.kpis
    ADD COLUMN IF NOT EXISTS viewed_by UUID REFERENCES public.users(id) ON DELETE SET NULL;
...
BEGIN
    ...
    WHERE user_id = auth.uid()
      AND viewed_by IS DISTINCT FROM auth.uid()
...
-- Supervisor edits must not look like the assignee opened the task.
