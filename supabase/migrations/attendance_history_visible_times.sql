-- History tables: fill shift / clock-out / duration from assignment + visit segments
-- when attendance_records.shift_id / clock_out_at / work_minutes are empty.
...
BEGIN
    ...
    FROM (
        SELECT
            ...
            COALESCE(
                NULLIF(ar.work_minutes, 0),
                NULLIF(vis.total_mins, 0),
                CASE
                    WHEN ar.clock_in_at IS NOT NULL AND COALESCE(ar.clock_out_at, vis.last_out) IS NOT NULL
                    THEN GREATEST(0, (EXTRACT(EPOCH FROM (COALESCE(ar.clock_out_at, vis.last_out) - ar.clock_in_at)) / 60)::INTEGER)
                    ELSE NULL
                END,
                CASE
                    WHEN ar.attendance_source IS DISTINCT FROM 'geo' THEN asg.shift_mins
                    ELSE NULL
                END
            ) AS rmins,
        ...
        LEFT JOIN LATERAL (
            SELECT
                ...
                SUM(vs.work_minutes)::INTEGER AS total_mins
